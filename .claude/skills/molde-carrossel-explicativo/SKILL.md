...
description: Gera carrosséis de Instagram no molde "Carrossel Explicativo" do @constitucionalgabaritado (6–7 slides: capa com foto em 5 layouts — marca-texto, revista, moldura, balão, manchete —, slides de conteúdo em 3 estilos — clássico, caderno, noturno —, CTA com selo). Use quando pedirem um carrossel para explicar um tema, dica, conceito, jurisprudência ou passo a passo (não a estrutura de peça em 3 slides, que é o molde Estrutura de Peças), ou quando colarem o bloco "CONTEÚDO" do prompt desse modelo.
---
...
2. Monte um bloco novo em `dados.json` (ou atualize o de mesmo `id`):
   ...
   - **Variação (obrigatória):** escolha `capa` (A, B, C, D ou F — não existe mais a E; o autor não gostou da capa tipográfica) e `estilo` (`classico`, `caderno`, `noturno`) olhando os últimos posts de `dados.json`: a capa não pode repetir a do post anterior e o estilo não pode repetir o dos dois anteriores. Prefira a capa que combina com o conteúdo: A (marca-texto) ou B (revista) para siglas/termos curtos, F (manchete) para temas "de notícia", D (balão) para pergunta ou dúvida no apoio, C (moldura) e B (revista) para títulos de 2 linhas, A (marca-texto) para títulos curtos e fortes. Posts gerados pelo `diario/` também precisam desses dois campos.
