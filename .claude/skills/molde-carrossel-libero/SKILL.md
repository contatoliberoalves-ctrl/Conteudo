...
2. Acrescente um objeto em `dados.json` com 6 a 10 slides, nesta estrutura:
   ...
   - **Foto de banco na capa**: rode `python3 buscar_imagens.py "termo em inglês"` (Openverse, só CC0/domínio público), veja a prancha em `build/busca/`, copie a escolhida para `imagens/`, registre o crédito em `imagens/CREDITOS.md` e use `"imagem"` na capa. Evite fotos de pessoas reais identificáveis (políticos, famosos) e prefira objetos, lugares e animais.
