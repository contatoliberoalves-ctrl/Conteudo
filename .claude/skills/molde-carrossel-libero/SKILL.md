...
2. Acrescente um objeto em `dados.json` com 6 a 10 slides, nesta estrutura:
   ...
   - **Objetos 3D** (`assets/`, fundo transparente): use 1 `objeto` na capa (canto) e 1 ou 2 `pontes` atravessando a divisa entre slides, ligados ao tema (lupa para investigação, balança para justiça, galinha para a Fuga das Galinhas…). Para baixar outros, veja "Objetos 3D" no README. 
   - **Foto de banco na capa**: rode `python3 buscar_imagens.py "termo em inglês"` (Openverse/Rawpixel, só CC0/domínio público), veja a prancha em `build/busca/` **olhando a imagem**, prepare a escolhida com `--preparar` (1080×1350), registre o crédito em `imagens/CREDITOS.md` e use `"imagem"` na capa: em tela cheia, com o título por cima (o autor prefere assim ao cartão). Escolha fotos bonitas e sem marca d'água; gravuras antigas combinam com o perfil. Evite fotos de pessoas reais identificáveis (políticos, famosos) e prefira objetos, lugares e animais.
